...
export function extractTitle( htmlString ) {
    const titleRegex = /<title(.*?)>((.|\s)*?)<\/title>/i;
    const match = htmlString.match( titleRegex );

    if ( match && match[2] ) {
        return decodeEntities( match[2].replace("\n","").replace("\r","").trim() );
    } else {
        return 'No title';
    }
}

export function DateYmdhis(date) {
...
export function decodeEntities(encodedString) {
    const specials = new RegExp(Object.keys(namedEntities).join('|'), 'g');

    return encodedString.replace(specials, function(match, entity) {
